...
const allowedKinds = new Set(['messages', 'user-avatars', 'server-icons', 'misc']);
...
export async function GET(_request: Request, ctx: { params: Promise<{ kind: string; name: string }> }) {
  try {
    ...
    if (!/^[a-z0-9_-]+-[0-9]+-[A-Za-z0-9]+\.(png|jpg|jpeg|gif|webp|zip)$/i.test(name)) return fail('CDN asset not found', 404);
